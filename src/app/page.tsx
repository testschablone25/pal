...
import { endOfWeek, parseISO, format, isToday } from "date-fns";
import {
	formatDate,
	formatDateShort,
	formatTime,
...
export default function DashboardPage() {
	...
	const todayTasks = tasks.filter((t) => {
		...
		return isDateToday(t.events.date);
	});
	...
	const todayShifts = shifts.filter((s) => {
		...
		return isDateToday((s as Shift).events!.date);
	});
										...
										<div>
											...
											<div className="space-y-2">
												{todayTasks.map((task) => (
													<div
														key={task.id}
														className="flex items-center gap-3 p-2 bg-zinc-800/50"
													>
										...
										<div>
											...
											<div className="space-y-2">
												{weekTasks.slice(0, 3).map((task) => (
													<div
														key={task.id}
														className="flex items-center gap-3 p-2 bg-zinc-800/30"
													>
														{getStatusIcon(task.status)}
														...
														<span className="text-xs text-zinc-500">
															{task.events?.date
																? formatDateShort(task.events.date)
																: ""}
														</span>
													</div>
												))}
									...
									{events.map((event) => {
										...
										return (
											<Link key={event.id} href={`/events/${event.id}`}>
												...
												>
													<div
														className={`text-center min-w-[50px] ${isEventToday ? "text-violet-400" : "text-zinc-400"}`}
													>
													...
													<div className="flex-1">
														<h4
															className={`font-medium ${isEventToday ? "text-white" : "text-zinc-200"}`}
														>
															{event.name}
														</h4>
										...
										<div>
											...
											<div className="space-y-2">
												{todayShifts.map((shift) => (
													<div
														key={shift.id}
														className="p-3 bg-green-600/10 border border-green-600/20"
													>
												...
												{upcomingShifts.slice(0, 2).map((shift) => (
													<div key={shift.id} className="p-2 bg-zinc-800/50">
														<div className="flex items-center justify-between mb-1">
															...
															<span className="text-xs text-zinc-500">
																{shift.events?.date
																	? formatDateShort(shift.events.date)
																	: ""}
															</span>
														</div>
					...
					<Card className="bg-zinc-900 border-zinc-800">
						...
						<CardContent>
							{colleagues.length === 0 ? (
								...
								<div className="space-y-3">
									{/* Group by event */}
									{[...new Set(colleagues.map((c) => c.event_id))]
										.slice(0, 2)
										.map((eventId) => {
											const eventColleagues = colleagues.filter(
												(c) => c.event_id === eventId,
											);
											const eventName =
												eventColleagues[0]?.events?.name || "Unknown";
											...
											return (
												<div key={eventId} className="space-y-2">
													<div className="flex items-center gap-2 text-sm">
														<span
															className={`font-medium ${isEventToday ? "text-violet-400" : "text-zinc-300"}`}
														>
															{eventName}
														</span>
														{isEventToday && (
													...
													<div className="flex flex-wrap gap-2">
														{eventColleagues.map((colleague) => {
															const name =
																...
																colleague.staff?.profiles?.email?.split(
																	"@",
																)[0] ||
																"Unknown";
															return (
																<div
																	key={colleague.id}
																	className="flex items-center gap-2 px-3 py-1.5 bg-zinc-800/50"
																>
						...
						<div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
							{canAccessRoute(userRoles, "/door") && (
								<Link href="/door">
									<Button
										variant="outline"
										className="w-full border-zinc-800 justify-start"
									>
