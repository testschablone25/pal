	...
	const handleDeleteGuest = async () => {
		...
		try {
			const response = await fetch(
				`/api/guest-lists/${params.id}/entries?entry_id=${deletingId}`,
				{ method: "DELETE" },
			);
					...
					<div>
						...
						{guestList.event && (
							<p className="text-zinc-400 mt-1">
								...
								{new Date(guestList.event.date).toLocaleDateString()}
							</p>
						)}
					</div>
				</div>
							...
							<div>
								...
								<Select
									...
									onValueChange={(v) =>
										setNewGuest({
											...newGuest,
											category: v as "presale" | "guestlist" | "walkin",
										})
									}
								>
							...
							<div>
								...
								<Input
									...
									onChange={(e) =>
										setNewGuest({
											...newGuest,
											plus_ones: parseInt(e.target.value) || 0,
										})
									}
									className="bg-zinc-950 border-zinc-800"
								/>
