...
// See review/findings.md (F5) for details.
// TODO: Either complete the feature (promoter roles, assignment UI, per-promoter tracking)
// or remove entirely. Currently dead code.
...
export async function POST(
	...
	try {
		...
		const qr_token_expires_at = new Date(
...
export async function DELETE(
	...
	try {
		...
		if (entry.guest_list_id !== listId) {
			return NextResponse.json(
				{ error: "Entry does not belong to this list" },
				{ status: 403 },
			);
		}
