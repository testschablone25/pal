...
function normalizeExtractedData(data: Record<string, unknown>): void {
  const hospitality = data.hospitality_rider as Record<string, unknown> | undefined;
  if (!hospitality) return;

  const transport = hospitality.transport_ground as Record<string, unknown> | undefined;
  if (!transport) return;
  ...
  if (hasTravelInfo && flightsNeeded === false) {
    console.log('[Normalize] Setting flights_needed to true due to travel info');
    transport.flights_needed = true;
  }

  // Fix: If there's accommodation info but required is not set, set it to true
  const accommodation = hospitality.accommodation as Record<string, unknown> | undefined;
  if (accommodation) {
    const hasAccommodationInfo = accommodation.room_type || accommodation.bed_type || accommodation.hotel_requirements;
    const required = accommodation.required as boolean | undefined;
    if (hasAccommodationInfo && required === false) {
      console.log('[Normalize] Setting accommodation.required to true due to accommodation info');
