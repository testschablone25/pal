...
import type { Task as TaskDetailTask } from "@/components/task-detail/types";
import {
	ClipboardList,
	CheckCircle2,
	AlertTriangle,
...
const TaskDetailDialog = dynamic(
	() =>
		import("@/components/task-detail-dialog").then((m) => ({
...
export function DashboardClient({ data }: { data: DashboardData }) {
	...
	const handleTaskClick = (task: Task) => {
		setSelectedTask(task as Task & Record<string, unknown> as never);
		setTaskDialogOpen(true);
	};

	const handleTaskUpdated = () => {
		// Re-fetch on the client — full page refresh for simplicity
		router.refresh();
	...
	return (
		<div className="min-h-screen bg-zinc-950">
			...
			{/* Task Detail Dialog */}
			<TaskDetailDialog
				task={selectedTask as unknown as TaskDetailTask}
				open={taskDialogOpen}
