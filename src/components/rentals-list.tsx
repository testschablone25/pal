...
import { Plus, Package, ArrowLeftFromLine, ArrowLeftRight } from "lucide-react";
...
export function RentalsList() {
	...
	const handleCreate = async (values: RentalFormValues) => {
		...
		fetchRentals();
		toast({
			title: "Ausleihe erstellt",
			description: `Ausleihe für ${values.rented_to} wurde erfolgreich erstellt.`,
		});
	};
	...
	const handleReturn = async (rental: Rental) => {
		...
		try {
			...
			fetchRentals();
			toast({
				title: "Ausleihe zurückgegeben",
				description: `${rental.items?.name || "Gegenstand"} wurde erfolgreich zurückgegeben.`,
			});
		} catch (error) {
			...
			toast({
				...
				title: "Fehler",
				description:
					error instanceof Error ? error.message : "Fehler bei der Rückgabe.",
			});
		} finally {
			setReturningId(null);
		}
			...
			<TableBody>
				{rentalsList.map((rental) => {
					...
					return (
						<TableRow
							key={rental.id}
							className={cn("border-zinc-800", overdue && "bg-red-900/10")}
						>
					...
					<Tabs defaultValue="active">
						...
						<TabsContent value="active" className="mt-4">
							...
							{loading ? (
								...
								<EmptyState
									icon={ArrowLeftRight}
									title="Keine aktiven Vermietungen"
									description="Erstelle eine neue Vermietung"
									actionLabel="Vermietung erstellen"
									onClick={() => setShowCreateDialog(true)}
								/>
							) : (
								<Card className="bg-zinc-900/70 backdrop-blur-sm border border-zinc-800/70 rounded-lg">
						...
						<TabsContent value="past" className="mt-4">
							<div className="flex justify-between items-center mb-4">
								<p className="text-sm text-zinc-400">
									{loading
										? "Loading..."
										: `${pastRentals.length} past rentals`}
								</p>
							</div>
