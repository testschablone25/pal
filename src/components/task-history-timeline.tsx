...
function getActionDescription(
	...
	switch (change_type) {
		...
		case "status_change":
			return t("history.status_change", {
				from:
					t(`status.${from_status}` as unknown as string) ||
					from_status?.replace(/_/g, " ") ||
					"unknown",
				to:
					t(`status.${to_status}` as unknown as string) ||
					to_status?.replace(/_/g, " ") ||
					"unknown",
			});
		case "approved":
		...
		case "rejected":
			return t("history.rejected", {
				reason: reason ?? t("history.no_reason"),
			});
		case "blocked":
			return t("history.blocked", { reason: reason ?? t("history.no_reason") });
		case "unblocked":
			return t("history.unblocked");
		case "edited":
			return t("history.edited", { reason: reason ?? t("history.no_reason") });
		default:
...
function formatRelativeTime(
	...
	if (diffMins < 60) return t("history.min_ago", { n: diffMins });
				...
				<div key={entry.id} className="relative pb-4 last:pb-0">
					...
					<div className="flex items-start gap-3">
						<Avatar className="h-7 w-7 mt-0.5 shrink-0">
							<AvatarImage
								src={entry.changed_by_profile?.avatar_url || undefined}
							/>
