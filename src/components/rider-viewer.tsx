...
import { RiderEditor } from "@/components/rider-editor";
...
export function RiderViewer({
	...
	const [riderType, setRiderType] = useState<"tech" | "hospitality" | "both">(
		"both",
	);
		...
		try {
			...
			if (response.ok) {
				const result = {
					...
					tasks_created:
						typeof data.tasks_created === "number" ? data.tasks_created : 0,
							...
							<DialogContent className="bg-zinc-900 border-zinc-800">
								...
								<div className="space-y-4">
									{openRouterStatus !== "connected" && (
										<div className="p-4 bg-yellow-950/50 border border-yellow-700 rounded-lg">
											<p className="text-yellow-400 text-sm flex items-center gap-2">
												...
												OpenRouter not connected. Upload works but extraction
												may be less accurate.
											</p>
											<p className="text-zinc-400 text-xs mt-2">
												Configure OpenRouter API key for improved extraction
												quality.
											</p>
										</div>
									)}
									...
									<div>
										...
										<Select
											value={riderType}
											onValueChange={(v) => setRiderType(v as typeof riderType)}
										>
											...
											<SelectContent className="bg-zinc-900 border-zinc-800">
												...
												<SelectItem value="tech">
													Technical Rider Only
												</SelectItem>
												<SelectItem value="hospitality">
													Hospitality Rider Only
												</SelectItem>
											</SelectContent>
										</Select>
									</div>
									...
									{uploadResult && (
										<div
											className={`p-4 rounded-lg ${uploadResult.success ? "bg-green-950/50 border border-green-700" : "bg-red-950/50 border border-red-700"}`}
										>
											{uploadResult.success ? (
												<div>
													...
													{uploadResult.task_events &&
														uploadResult.task_events.length > 0 && (
															<div className="mt-2 space-y-1">
																{uploadResult.task_events.map(
																	(event, index) => (
																		<p
																			key={
																				event.event_id || `unscheduled-${index}`
																			}
																			className="text-zinc-300 text-xs"
																		>
																			{event.event_name} ({event.event_date}):{" "}
																			{event.tasks_created} task(s)
																		</p>
																	),
																)}
															</div>
									...
									{uploading && (
										<p className="text-xs text-zinc-500 text-center mt-2">
											Local AI is processing your PDF. This may take a few
											minutes.
										</p>
									)}
								</div>
				...
				<CardContent>
					{!hasRiders ? (
						<div className="text-center py-8 text-zinc-500">
							...
							<p className="text-sm">
								Upload a PDF to extract rider requirements
							</p>
						</div>
					) : (
							...
							<TabsContent value="tech" className="mt-4 space-y-4">
									...
									<div className="p-3 bg-blue-950/30 border border-blue-800/50 rounded-lg">
										...
										<div className="text-sm text-zinc-400 space-y-1">
											{(techRider.transport.origin_city ||
												techRider.transport.flights_needed) && (
												<p>
													Origin: {techRider.transport.origin_city || "Unknown"}
												</p>
											)}
											{techRider.transport.baggage_requirements && (
												<p>
													Baggage: {techRider.transport.baggage_requirements}
												</p>
											)}
											{!techRider.transport.flights_needed &&
												techRider.transport.priority_boarding && (
									...
									<div className="p-3 bg-zinc-800/50 rounded-lg">
										...
										<div className="text-sm text-zinc-400 space-y-1">
											...
											{techRider.audio.preferred_mixers &&
												techRider.audio.preferred_mixers.length > 0 && (
													<div className="pt-2">
								...
								{hasStageSetup && techRider?.stage_setup && (
									<div className="p-3 bg-cyan-950/30 border border-cyan-800/50 rounded-lg">
										...
										{techRider.stage_setup.monitors &&
											techRider.stage_setup.monitors.length > 0 && (
												<div className="mb-3">
													<p className="text-xs font-medium text-cyan-300 mb-1">
														Monitors
													</p>
													<div className="space-y-1">
														{techRider.stage_setup.monitors.map(
															(monitor, index) => (
																<p
																	key={`${monitor.type}-${index}`}
																	className="text-sm text-zinc-300"
																>
																	{monitor.type} x{monitor.quantity}{" "}
																	{monitor.location
																		? `(${monitor.location})`
																		: ""}
																</p>
										...
										{techRider.stage_setup.power &&
											techRider.stage_setup.power.length > 0 && (
												<div className="mb-3">
													<p className="text-xs font-medium text-cyan-300 mb-1">
														Power
													</p>
										...
										{techRider.stage_setup.furniture &&
											techRider.stage_setup.furniture.length > 0 && (
												<div>
													<p className="text-xs font-medium text-cyan-300 mb-1">
														Furniture
													</p>
													<div className="space-y-1">
														{techRider.stage_setup.furniture.map(
															(item, index) => (
																<p
																	key={`${item.type}-${index}`}
																	className="text-sm text-zinc-300"
								...
								{hasMixerRequirements && (
									<div className="p-3 bg-emerald-950/30 border border-emerald-800/50 rounded-lg">
										...
										{techRider?.backline?.cdjs &&
											techRider.backline.cdjs.length > 0 && (
												<div className="mb-2">
										...
										{techRider?.backline?.turntables &&
											techRider.backline.turntables.length > 0 && (
												<div>
													<p className="text-xs font-medium text-emerald-300 mb-1">
														Turntables
													</p>
								...
								{techRider?.equipment && techRider.equipment.length > 0 && (
									<div className="space-y-4">
										{/* Venue Must Supply */}
										{techRider.equipment.filter((e) => !e.artist_brings)
											.length > 0 && (
											<div>
												...
												<div className="space-y-2">
													{techRider.equipment
														.filter((e) => !e.artist_brings)
														.map((item, i) => (
															<div
																key={i}
																className="flex items-center justify-between p-2 bg-red-950/20 border border-red-900/30 rounded"
															>
																<div>
										...
										{techRider.equipment.filter(
											(e) =>
												e.artist_brings &&
												!e.notes?.toLowerCase().includes("optional"),
										).length > 0 && (
											<div>
												...
												<div className="space-y-2">
													{techRider.equipment
														...
														.map((item, i) => (
															<div
																key={i}
																className="flex items-center justify-between p-2 bg-green-950/20 border border-green-900/30 rounded"
															>
																<div>
																	<span className="text-sm text-white">
																		{item.name}
												...
												<div className="space-y-2">
													{techRider.equipment
														...
														.map((item, i) => (
															<div
																key={i}
																className="flex items-center justify-between p-2 bg-yellow-950/20 border border-yellow-900/30 rounded"
															>
																<div>
																	<span className="text-sm text-white">
																		{item.name}
								...
								{techRider?.performance_requirements && (
									<div className="p-3 bg-indigo-950/30 border border-indigo-800/50 rounded-lg">
												...
												<div className="space-y-1">
													{techRider.performance_requirements.staff
														...
														<div className="flex items-center gap-2 text-sm text-zinc-300">
															...
															{techRider.performance_requirements.staff
																.sound_tech_notes &&
																`(${techRider.performance_requirements.staff.sound_tech_notes})`}
														</div>
													)}
													{techRider.performance_requirements.staff
														?.lighting_tech && (
														<div className="flex items-center gap-2 text-sm text-zinc-300">
															...
															{techRider.performance_requirements.staff
																.lighting_tech_notes &&
																`(${techRider.performance_requirements.staff.lighting_tech_notes})`}
														</div>
													)}
												</div>
											</div>
										...
										{/* Stage Requirements */}
										{techRider.performance_requirements.stage?.requirements &&
											techRider.performance_requirements.stage.requirements
												.length > 0 && (
												<div>
													<p className="text-xs font-medium text-indigo-300 mb-1">
													...
													<div className="space-y-1">
														{techRider.performance_requirements.stage.requirements.map(
															(req, i) => (
																<div
																	key={i}
								...
								{techRider?.referenced_images &&
									techRider.referenced_images.length > 0 && (
										<div className="p-3 bg-zinc-800/30 rounded-lg">
											<p className="text-sm font-medium text-zinc-300 mb-2">
												Referenced Images / Visual Notes
											</p>
											<div className="space-y-1">
							...
							<TabsContent value="hospitality" className="mt-4 space-y-4">
								...
								{hospitalityRider?.accommodation && (
									<div className="p-3 bg-purple-950/30 border border-purple-800/50 rounded-lg">
										...
										<div className="text-sm text-zinc-400 space-y-1">
											<p>
												{hospitalityRider.accommodation.nights > 0
													? `${hospitalityRider.accommodation.nights} night(s), `
													: ""}
												{hospitalityRider.accommodation.room_type ||
													"Accommodation requested"}
											</p>
											{(hospitalityRider.accommodation.check_in ||
												hospitalityRider.accommodation.check_out) && (
												<p>
													Check-in:{" "}
													{hospitalityRider.accommodation.check_in || "n/a"} /
													Check-out:{" "}
													{hospitalityRider.accommodation.check_out || "n/a"}
												</p>
											)}
									...
									<div className="p-3 bg-orange-950/30 border border-orange-800/50 rounded-lg">
										...
										<div className="text-sm text-zinc-400 space-y-1">
											{hospitalityRider.catering.meals?.length > 0 && (
												<p>
													Meals: {hospitalityRider.catering.meals.join(", ")}
												</p>
											)}
											{hospitalityRider.catering.dietary?.length > 0 && (
												<p>
													Dietary:{" "}
													{hospitalityRider.catering.dietary.join(", ")}
												</p>
											)}
											{hospitalityRider.catering.drinks.spirits?.length > 0 && (
												<p>
													Spirits:{" "}
													{hospitalityRider.catering.drinks.spirits.join(", ")}
												</p>
											)}
											{hospitalityRider.catering.drinks.mixers?.length > 0 && (
												<p>
													Mixers:{" "}
													{hospitalityRider.catering.drinks.mixers.join(", ")}
												</p>
											)}
								...
								{!hasHospitality && (
									<div className="p-3 bg-zinc-800/30 rounded-lg">
										<p className="text-sm text-zinc-500">
											No hospitality requirements extracted yet.
										</p>
									</div>
								)}
							</TabsContent>
					...
					<DialogContent className="bg-zinc-900 border-zinc-800">
						...
						<div className="space-y-4">
							<p className="text-zinc-400">
								This will delete all rider data for{" "}
								<strong className="text-white">{artistName}</strong> including:
							</p>
							...
							{uploadResult && (uploadResult.tasks_created || 0) > 0 ? (
										...
										uploadResult.task_events.length > 0 && (
											<div className="space-y-4">
												{uploadResult.task_events.map((event, eventIndex) => (
													<div
														key={event.event_id || `event-${eventIndex}`}
													...
													>
														...
														{event.task_titles &&
															event.task_titles.length > 0 && (
																<ul className="space-y-1">
																	{event.task_titles.map(
																		(title, titleIndex) => (
																			<li
																				key={titleIndex}
								...
								<div className="text-center py-8">
									...
									<p className="text-white font-medium">
										Rider data extracted!
									</p>
									<p className="text-zinc-400 text-sm mt-1">
										No tasks were created - you can manually create tasks in the
										Workflow page.
									</p>
									<Button
										onClick={() => {
					...
					<DialogContent className="bg-zinc-900 border-zinc-800 max-w-2xl">
						...
						<div className="space-y-4 max-h-96 overflow-y-auto">
							<p className="text-zinc-500 text-sm">
								When you upload a new rider PDF, the previous rider is
