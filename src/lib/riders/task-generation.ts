...
function isRiderTaskTitleForArtist(title: string, artistName: string): boolean {
	return (
		...
		RIDER_TITLE_KEYWORDS.some((keyword) => title.includes(keyword))
	);
}
...
function buildTaskDrafts(
	...
	if (hasFlights) {
		...
		const travelNotes = (transportData as Record<string, unknown>)
			?.travel_booking_notes as string | undefined;
...
4. Send itinerary to artist management`,
	...
	const venueSupplies =
		techRider?.equipment?.filter((item) => !item.artist_brings) || [];
	const artistBrings =
		techRider?.equipment?.filter(
			(item) => item.artist_brings && !isOptionalNote(item.notes),
		) || [];
	const artistOptional =
		techRider?.equipment?.filter(
			(item) => item.artist_brings && isOptionalNote(item.notes),
		) || [];
	...
	if (stageSetup?.monitors?.length) {
		venueEquipmentList += `\n\nSTAGE MONITORS:\n${stageSetup.monitors
			.map(
				(monitor) =>
					`- ${monitor.type}: ${monitor.quantity} (${monitor.location})`,
			)
			.join("\n")}`;
	}
	...
	if (stageSetup?.furniture?.length) {
		venueEquipmentList += `\n\nFURNITURE:\n${stageSetup.furniture
			.map(
				(item) =>
					`- ${item.type}: ${item.quantity}${item.dimensions ? ` (${item.dimensions})` : ""}`,
			)
			.join("\n")}`;
	}
	...
	const hasAudioSetup =
		(audioInfo?.inputs_needed && audioInfo.inputs_needed > 2) ||
	...
	if (hasAudioSetup) {
		const mixerList = audioInfo.preferred_mixers?.length
			? audioInfo.preferred_mixers
					...
					.map(
						(m) =>
							`${m.priority === 1 ? "✓" : m.priority === 2 ? "○" : "○"} ${m.model}${m.required_features ? ` (${m.required_features})` : ""}`,
					)
					.join("\n")
			: "Not specified";
...
4. Test audio before artist soundcheck`,
	...
	if (hasStageSetup) {
		...
		const furnitureList = stageSetupData.furniture?.length
			? stageSetupData.furniture
					.map(
						(f) =>
							`- ${f.type}: ${f.quantity}${f.dimensions ? ` (${f.dimensions})` : ""}`,
					)
					.join("\n")
			: "None";
...
4. Check referenced images in rider PDF`,
	...
	if (hasBackline) {
		...
		const turntablesList = backlineData.turntables?.length
			? backlineData.turntables
					.map((t) => `- ${t.model}: ${t.quantity}`)
					.join("\n")
			: "None";
...
3. Test equipment before soundcheck`,
	...
	if (venueSupplies.length > 0) {
		...
		const artistListText =
			artistBrings.length > 0
				? artistBrings.map((i) => `- ${i.name} (x${i.quantity})`).join("\n")
				: "None";
		const optionalListText =
			artistOptional.length > 0
				? artistOptional
						.map(
							(i) =>
								`- ${i.name} (x${i.quantity})${i.notes ? ` (${i.notes})` : ""}`,
						)
						.join("\n")
				: "None";
...
3. Notify booker if anything is missing`,
	...
	console.log(
		"[TaskGen] performance_requirements:",
		JSON.stringify(perfReqs?.staff, null, 2),
	);

	if (perfReqs?.staff?.sound_tech) {
		console.log(
			"[TaskGen] Sound tech detected! soundcheck:",
			perfReqs.staff.soundcheck_required,
			"set:",
			perfReqs.staff.set_required,
		);
		...
		const hasGranularStaffInfo =
			perfReqs.staff.soundcheck_required || perfReqs.staff.set_required;
		console.log(
			"[TaskGen] hasSpecificTime:",
			Boolean(perfReqs.staff.specific_time),
			"hasParty:",
			Boolean(perfReqs.staff.party_mentioned),
			"hasGranularStaffInfo:",
			hasGranularStaffInfo,
		);

		if (!hasGranularStaffInfo) {
			console.log(
				"[TaskGen] Creating fallback sound technician task (needs_refining: true)",
			);
			tasks.push({
...
3. Get exact timing from artist management`,
		...
		if (perfReqs.staff.soundcheck_required) {
			const needsRefining =
				...
				!Boolean(perfReqs.staff.party_mentioned);
			console.log(
				"[TaskGen] Creating soundcheck task, needs_refining:",
				needsRefining,
			);
			tasks.push({
...
- Return: ${hospitalityRider.transport_ground.return_required ? "Required" : "Not needed"}`,
	...
	if (
		hospitalityRider?.catering?.dietary?.length ||
...
async function buildRoleContext(
	supabase: SupabaseClient,
): Promise<RoleContext> {
	...
	return {
		managerId,
		bookerId,
		soundStaff: staffRows.filter(
			(row) => row.role === "sound" && Boolean(row.profile_id),
		),
		lightStaff: staffRows.filter(
			(row) => row.role === "light" && Boolean(row.profile_id),
		),
	};
}
...
async function getAvailableStaffForDate(
	...
	const availableIds = new Set(
		rows.filter((row) => row.available).map((row) => row.staff_id),
	);
...
async function resolveAssignment(
	...
	if (assignmentTarget === "booker") {
		return {
			...
			warning: roleContext.bookerId
				? undefined
				: "No booker user role found for assignment",
	...
	if (assignmentTarget === "manager") {
		return {
			...
			warning: roleContext.managerId
				? undefined
				: "No manager user role found for assignment",
			needsDelegation: false,
		};
	}
	...
	const candidates =
		targetRole === "light" ? roleContext.lightStaff : roleContext.soundStaff;
	const availableCandidates = await getAvailableStaffForDate(
		supabase,
		candidates,
		eventDate,
	);
...
async function cleanupExistingRiderTasks(
	...
	const query = supabase.from("tasks").select("id, title, description");
...
async function createTasksForEvent(
	...
	for (const draft of drafts) {
		...
		if (error) {
			eventWarnings.push(
				`Failed to create task "${draft.title}": ${error.message}`,
			);
			continue;
		}
		...
		if (
			insertedTask &&
			isRecord(insertedTask) &&
			typeof insertedTask.title === "string"
		) {
		...
		if (assignment.needsDelegation) {
			...
			if (delegationTitle) {
				...
				eventWarnings.push(
					`Could not create manager delegation task for "${draft.title}"`,
				);
			}
		}
	}
...
async function resolveTargetEvents(
	...
	if (explicitEventId) {
		...
		if (!data || !isRecord(data)) {
			return {
				events: [],
				warnings: ["Specified event for task generation was not found"],
			};
		}
	...
	const performanceRows = (
		Array.isArray(performancesData) ? performancesData : []
	) as PerformanceRow[];
	const eventIds = [
		...new Set(
			performanceRows
	...
	if (eventIds.length === 0) {
		...
		return {
			...
			warnings: [
				"No performances linked to this artist; tasks were created as unscheduled booking tasks",
			],
		};
	}
	...
	if (upcomingEvents.length === 0) {
		...
		return {
			...
			warnings: [
				"No upcoming events found for this artist; tasks were created as unscheduled booking tasks",
			],
		};
	}
...
export async function generateRiderTasksForArtist(
	...
	const totalTasks = eventResults.reduce(
		(sum, event) => sum + event.tasks_created,
		0,
	);
